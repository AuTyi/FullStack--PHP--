...
if (is_numeric($edit) && !$_POST) {
        ...
        $data = $conn->query("SELECT * FROM `blog_blog` WHERE id=$edit");//select from table blog
		  ...
		  if(strlen($title) == 0){
            ...
            $content = $conn->real_escape_string($content);
               ...
               if(is_numeric($edit)) {
                        ...
                        $saved = $conn->query("UPDATE `blog_blog` SET `title`='$title', `author`='$author', `content`='$content' 
                        WHERE id='$edit'"); 
                           
                 }else{
                      $saved = $conn->query("INSERT INTO `blog_blog`(`title`, `author`, `content`) 
...
<body>
    ...
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>"> <!--//add htmlspecialchars to avoid exploits input;-->
